# Code reused from https://github.com/tianlinyang/DKVMN
import torch
...
class DKVMNHeadGroup(nn.Module):
    ...
    def write(self, control_input, memory, write_weight=None, updated_weight=None):
        """
        Parameters
            control_input:      Shape (batch_size, control_state_dim)
        ...
        add_signal = self.add(control_input)
        add_reshape = add_signal.view(-1, 1, self.memory_state_dim)
        weight_reshape = updated_weight.view(-1, 1, 1)
        write_weight_reshape = write_weight.view(-1, self.memory_size, 1)
        weight_mult = torch.mul(weight_reshape, write_weight_reshape)
        add_mul = torch.mul(add_reshape, write_weight_reshape)
        new_memory = memory * (1 - weight_mult) + weight_mult * add_mul
...
class DKVMN(nn.Module):
    ...
    def write(self, write_weight, control_input, if_write_memory, updated_weight):
